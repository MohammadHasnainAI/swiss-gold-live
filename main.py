...
# ---------------------------------------------------------
# AUTO-REFRESH (Runs for EVERYONE)
# ---------------------------------------------------------
...
# --- FIX: SAFE SAVE FUNCTION (Prevents SHA Error) ---
def save_to_github(filename, data, message="Update"):
    if not repo: return False
    try:
        # Fetch fresh content to get the latest SHA
...
# 4. HYBRID DATA ENGINE (Twelve Data + Yahoo)
@st.cache_data(ttl=120, show_spinner=False)
def get_live_rates():
    data = {
        "gold": 2750.0, "silver": 32.0, "usd": 278.0, "aed": 3.67, 
        "time": "Offline", "full_date": datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    }

    # 1. GOLD (Twelve Data)
    if "TWELVE_DATA_KEY" in st.secrets:
        try:
            url = f"https://api.twelvedata.com/price?symbol=XAU/USD&apikey={st.secrets['TWELVE_DATA_KEY']}"
            res = requests.get(url, timeout=5).json()
            if 'price' in res:
                data['gold'] = float(res['price'])
        except: pass

        # Currencies
        try:
            url_curr = f"https://v6.exchangerate-api.com/v6/{st.secrets['CURR_KEY']}/latest/USD"
            res_curr = requests.get(url_curr, timeout=5).json()
            data['usd'] = float(res_curr.get('conversion_rates', {}).get('PKR', 278.0))
            data['aed'] = float(res_curr.get('conversion_rates', {}).get('AED', 3.67))
        except: pass

    # 2. SILVER (Yahoo Finance - Free & Unlimited)
    try:
        # XAG-USD is the standard symbol for Silver Spot
        ticker = yf.Ticker("XAG-USD")
        hist = ticker.history(period="1d")
        if not hist.empty:
            data['silver'] = float(hist['Close'].iloc[-1])
    except: pass

    data['time'] = datetime.now(pytz.timezone("Asia/Karachi")).strftime("%I:%M %p")
    data['full_date'] = datetime.now(pytz.timezone("Asia/Karachi")).strftime("%Y-%m-%d %H:%M:%S")
    return data
    ...
    with tabs[0]:
        ...
        if st.button("🚀 SAVE CHANGES", type="primary", use_container_width=True):
            if repo:
                ...
                save_to_github("history.json", hist_data, "Log History")
