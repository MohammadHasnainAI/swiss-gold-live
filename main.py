...
# 1. PAGE CONFIG
st.set_page_config(page_title="Islam Jewellery V14", page_icon="💎", layout="centered")
...
# 5. SETTINGS ENGINE - FIXED CACHE TIMING
@st.cache_data(ttl=10, show_spinner=False)  # Increased from 3s for stability
def load_settings():
    """Load settings from GitHub with optimized cache timing"""
...
# 6. DATA ENGINE - FIXED API URLS
@st.cache_data(ttl=15, show_spinner=False)  # Reduced from 30s for better freshness
def get_live_rates():
    """Get live market rates with fixed API URLs"""
    ...
    try:
        ...
        # FIXED: Removed spaces in URLs that caused API failures
        ...
        if 'XAU/USD' not in metal_res or 'price' not in metal_res['XAU/USD']:
            raise ValueError("Invalid gold API response")
        if 'XAG/USD' not in metal_res or 'price' not in metal_res['XAG/USD']:
            raise ValueError("Invalid silver API response")
        
        gold_price = float(metal_res['XAU/USD']['price'])
        silver_price = float(metal_res['XAG/USD']['price'])
        
        # Validate currency response
        if 'conversion_rates' not in curr_res:
            raise ValueError("Invalid currency API response")
        
        return {
            "gold": gold_price,
            "silver": silver_price,
            "usd": float(curr_res['conversion_rates'].get('PKR', 278.0)),
            "aed": float(curr_res['conversion_rates'].get('AED', 3.67)),
        ...
        st.error(f"API Error: {e} (Using demo values)")
        return {
            "gold": 2750.0, "silver": 32.0, "usd": 278.0, "aed": 3.67, 
            "time": "Error", 
            "full_date": datetime.now().strftime("%Y-%m-%d %H:%M:%S")
...
# 8. SESSION STATE MANAGEMENT - FIXED SYNC ISSUES
...
if current_update_time > st.session_state.last_seen_update:
    ...
    if not st.session_state.get("is_admin_publishing", False):
        ...
        # Update session state with confirmed values
        st.session_state.new_gold = int(settings.get("gold_premium", 0))
        st.session_state.new_silver = int(settings.get("silver_premium", 0))
...
try:
    ...
    if not (2000 <= gold_ounce <= 4000):
        st.warning(f"Gold ounce price ${gold_ounce:.2f} seems unusual. Using fallback.")
        gold_ounce = 2750.0
    
    # Get premiums from settings (authoritative source)
...
if st.session_state.admin_auth:
    ...
    with tabs[1]:
        ...
        # FIXED: Clarified labels
        st.info(f"**Gold/oz:** ${gold_ounce:,.2f} | **USD Rate:** Rs {usd_rate:.2f} | **Premium:** Rs {int(gold_premium):,}")
        ...
        with stats_cols[2]:
            st.markdown(f'<div style="background: linear-gradient(135deg, #d4af37 0%, #f4e5c2 100%); color: #1a1a1a; border-radius: 12px; padding: 1.5rem; text-align: center;"><div style="font-size: 2rem;">💵</div><div style="font-size: 0.8rem; text-transform: uppercase; letter-spacing: 1px; opacity: 0.9; margin-top: 0.5rem;">USD Rate</div><div style="font-size: 1.4rem; font-weight: 800;">Rs {usd_rate:.2f}</div></div>', unsafe_allow_html=True)
    ...
    # TAB 3: History (improved error handling)
    with tabs[2]:
...
<small>Data sources: TwelveData & ExchangeRate-API</small>
</div>
""", unsafe_allow_html=True)
